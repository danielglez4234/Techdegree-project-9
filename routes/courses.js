...
router.get('/courses', asyncHandler(async (req, res) => {
  ...
  if (courses === null) { // if no course is found
    res.status(400).json({ Error: 'the courses cannot be found' });
  }
...
router.get('/courses/:id', asyncHandler(async (req, res) => {
  ...
  if (courses === null) { // if no course is found
    res.status(400).json({ Error: 'the course cannot be found' });
  }
  res.status(200).json({ courses });
}));
...
router.post('/courses', [
  ...
  .withMessage("Description is required, Please provide a value for 'description'")
    ...
    if (!errors.isEmpty()) {
      ...
      res.status(400).json({ Error: errorMessages }); // Return errors to the client
    }else {
      let course;
      try {
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      ...
      if (course) { // verify that the course to be updated exists
        ...
        res.status(400).json({ message: 'This course ID doesnt exists' });
      }
    } catch (error) {
      course = await Course.build(req.body);
      course.id = req.params.id; // make sure correct course gets updated
      res.status(400).json({ message: 'failed to update the course' });
    }
  }
}));
...
router.delete('/courses/:id', authenticateUser, asyncHandler(async (req ,res) => {
  ...
  if(course) { // verify that the course to be deleted exists
    if(course.userId === req.currentUser.id) { // verify if the current user own the requested course
      ...
      res.status(403).json({ message: 'You can only delete the courses you own' });
    }
  } else {
    res.status(400).json({ message: 'this course ID doesnt exists' });
  }
